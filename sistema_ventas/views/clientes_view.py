...
class ClientesView(tk.Frame):
    ...
    def _build(self):
        ...
        card = make_card(self)
        cols = ['#', 'Nombre', 'Celular', 'CI/NIT', 'Dirección', 'Categoría', 'Desc.Cat.']
        keys = ['id', 'nombre', 'celular', 'ci_nit', 'direccion', 'categoria', 'descuento_cat']
        self._keys = keys
        frame_tree, self.tree = make_treeview(card, cols)
        frame_tree.pack(fill=BOTH, expand=True, padx=10, pady=10)
        widths = {'#': 40, 'Nombre': 160, 'Celular': 90, 'CI/NIT': 90, 'Dirección': 140, 'Categoría': 110, 'Desc.Cat.': 70}
        for col in cols:
...
class ClienteDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        rows_fields = [("Nombre *", self._nombre, False, None),
                       ...
                       ("Dirección", self._direccion, False, None),
        ...
        if self.cliente:
            ...
            self._direccion.set(self.cliente.get('direccion',''))
    ...
    def _guardar(self):
        ...
        data = {'nombre': self._nombre.get(), 'celular': self._celular.get(),
                'ci_nit': self._ci_nit.get(), 'direccion': self._direccion.get(),
                'categoria_cliente_id': cat['id'] if cat else None}

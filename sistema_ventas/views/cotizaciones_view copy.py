...
from views.ventas_view import calcular_precio_con_descuento
...
class CotizacionesView(tk.Frame):
    ...
    def _export_pdf(self):
        ...
        path = exportar_cotizacion_pdf(cot, detalles, cot.get('cliente',''))
...
class NuevaCotizacionPanel(tk.Frame):
    ...
    def _build(self):
        ...
        tb.Button(right, text="🗑️ Quitar seleccionado", bootstyle="danger-outline",
                  command=self._quitar).pack(anchor='w', pady=(5, 0))
    ...
    def _agregar(self):
        ...
        self.items.append({
            ...
            'pct_descuento':        pct,
            'cantidad':             cant,
            # Para guardar en BD (cotizaciones usa 'precio')
            'precio':               precio_final,
        })
        self._refresh()
        self.cant_var.set('1')
    ...
    def _quitar(self):
        ...
        if sel:
            self.items.pop(self.cart_tree.index(sel[0]))
            self._refresh()
    ...
    def _guardar(self):
        ...
        cot_id = registrar_cotizacion(cliente['id'], self.items)

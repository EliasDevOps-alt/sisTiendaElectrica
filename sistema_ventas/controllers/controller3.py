...
def save_cliente(data, id=None):
    conn = get_connection()
    if id:
        conn.execute("UPDATE clientes SET nombre=?,celular=?,ci_nit=?,categoria_cliente_id=? WHERE id=?",
                     (data['nombre'], data['celular'], data['ci_nit'], data['categoria_cliente_id'], id))
    else:
        conn.execute("INSERT INTO clientes (nombre,celular,ci_nit,categoria_cliente_id) VALUES (?,?,?,?)",
                     (data['nombre'], data['celular'], data['ci_nit'], data['categoria_cliente_id']))
    conn.commit(); conn.close()
...
# ─── COMPRAS ───────────────────────────────────────────────────
def registrar_compra(proveedor_id, detalles):
    """detalles: list of {producto_id, cantidad, costo_unitario}"""
    conn = get_connection()
    total_compra = sum(d['cantidad'] * d['costo_unitario'] for d in detalles)
    cursor = conn.execute("INSERT INTO compras (proveedor_id, total) VALUES (?, ?)", (proveedor_id, total_compra))
    compra_id = cursor.lastrowid
    for d in detalles:
        total = d['cantidad'] * d['costo_unitario']
        conn.execute("INSERT INTO detalle_compras (compra_id,producto_id,cantidad,costo_unitario,total) VALUES (?,?,?,?,?)",
                     (compra_id, d['producto_id'], d['cantidad'], d['costo_unitario'], total))
    # Registrar egreso en caja
    conn.execute("INSERT INTO caja (tipo,monto,descripcion,referencia_id) VALUES ('EGRESO',?,?,?)",
...
def get_ventas():
    conn = get_connection()
    # Agregar columna descuento_total si no existe (compatibilidad)
    try:
        conn.execute("ALTER TABLE ventas ADD COLUMN descuento_total REAL DEFAULT 0")
        conn.commit()
    except: pass
...
def get_detalle_venta(venta_id):
    conn = get_connection()
    rows = conn.execute("""
        SELECT dv.*, p.descripcion, p.codigo_tienda,
...
def registrar_cotizacion(cliente_id, detalles):
    ...
    cursor = conn.execute("INSERT INTO cotizaciones (cliente_id,total) VALUES (?,?)", (cliente_id, total))
    cot_id = cursor.lastrowid
    for d in detalles:
        conn.execute("INSERT INTO detalle_cotizaciones (cotizacion_id,producto_id,cantidad,precio,total) VALUES (?,?,?,?,?)",
                     (cot_id, d['producto_id'], d['cantidad'], d['precio'], d['cantidad']*d['precio']))
    conn.commit(); conn.close()
    return cot_id
...
def get_cotizaciones():
    conn = get_connection()
    rows = conn.execute("""
        SELECT q.id, q.fecha, q.estado, q.total, c.nombre as cliente
...
def get_detalle_cotizacion(cot_id):
    ...
    rows = conn.execute("""
        SELECT dc.*, p.descripcion, p.codigo_tienda
        FROM detalle_cotizaciones dc JOIN productos p ON dc.producto_id = p.id
